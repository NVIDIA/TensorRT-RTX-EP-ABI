...
TEST_P(TypeTests, IOTypes)
{
    ...
    const std::string model_name = "nv_execution_provider_" + dtype_name + ".onnx";

    model_builder::CreateBaseModel(model_name, "test_" + dtype_name, {1, 5, 10}, false, dtype);
    {
        Ort::SessionOptions so;
        AppendTrtRtxEp(so);
        Ort::Session session(*ort_env, toOrtString(model_name).c_str(), so);
...
INSTANTIATE_TEST_SUITE_P(TensorRTRTXEpTest, TypeTests,
                         ::testing::Values(ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT, ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT16,
                                           ONNX_TENSOR_ELEMENT_DATA_TYPE_BFLOAT16, ONNX_TENSOR_ELEMENT_DATA_TYPE_INT64,
...
TEST(TensorRTRTXEpTest, GetSharedAllocator)
{
    ...
    const OrtMemoryInfo* device_mem_info = c_api.EpDevice_MemoryInfo(devices[0], OrtDeviceMemoryType_DEFAULT);

    OrtAllocator* allocator = nullptr;
    Ort::ThrowOnError(c_api.GetSharedAllocator(*ort_env, device_mem_info, &allocator));
    ASSERT_NE(allocator, nullptr);

    const OrtMemoryInfo* host_mem_info = c_api.EpDevice_MemoryInfo(devices[0], OrtDeviceMemoryType_HOST_ACCESSIBLE);
...
TEST(TensorRTRTXEpTest, LoadUnloadPluginLibrary)
{
    ...
    Ort::ThrowOnError(c_api.UnregisterExecutionProviderLibrary(*ort_env, kEpName));

    // Register
    Ort::ThrowOnError(c_api.RegisterExecutionProviderLibrary(*ort_env, kEpName, toOrtString(g_ep_lib_path).c_str()));
    ...
    auto count = std::count_if(ep_devices, ep_devices + num_devices,
                               [&](const OrtEpDevice* d)
                               {
...
TEST(TensorRTRTXEpTest, LoadUnloadPluginLibraryCxxApi)
{
    ...
    ASSERT_NE(it, ep_devices.end()) << "Expected an OrtEpDevice for TRT RTX EP.";
...
TEST(TensorRTRTXEpTest, DataTransfer)
{
    ...
    Ort::Value cpu_tensor = Ort::Value::CreateTensor<float>(cpu_allocator.GetInfo(), input_data.data(),
                                                            input_data.size(), shape.data(), shape.size());

    // Create GPU tensor using shared allocator
    OrtAllocator* gpu_allocator = nullptr;
    Ort::ThrowOnError(c_api.GetSharedAllocator(*ort_env, device_mem_info, &gpu_allocator));
    ASSERT_NE(gpu_allocator, nullptr);

    Ort::Value device_tensor = Ort::Value::CreateTensor<float>(gpu_allocator, shape.data(), shape.size());

    // Copy CPU -> GPU
    {
        const OrtValue* src[] = {cpu_tensor};
        OrtValue* dst[] = {device_tensor};
        Ort::ThrowOnError(c_api.CopyTensors(*ort_env, src, dst, nullptr, 1));
    }

    // Copy GPU -> CPU (new tensor)
    Ort::Value cpu_copy = Ort::Value::CreateTensor<float>(cpu_allocator, shape.data(), shape.size());
    {
    ...
    {
        EXPECT_FLOAT_EQ(original[i], copied[i]) << "Mismatch at index " << i;
    }
...
TEST(TensorRTRTXEpTest, AutoEp_PreferGpu)
{
    ...
    ASSERT_FALSE(devices.empty()) << "TRT RTX EpDevice not visible in env; PREFER_GPU has nothing to pick.";
...
TEST(TensorRTRTXEpTest, FP8CustomOpModel)
{
    ...
    const std::string model_name = "nv_execution_provider_fp8_quantize_dequantize_test.onnx";
    clearFileIfExists(model_name);

    model_builder::CreateFP8CustomOpModel(model_name, "nv_execution_provider_fp8_quantize_dequantize_graph");
    ...
    for (size_t i = 0; i < num_elements; ++i)
    {
        input_data[i] = model_builder::Float32ToFloat16(static_cast<float>(i % 100) / 100.0f);
    }

    Ort::AllocatorWithDefaultOptions cpu_allocator;
    Ort::Value input_tensor =
        Ort::Value::CreateTensor(cpu_allocator.GetInfo(), input_data.data(), input_data.size() * sizeof(uint16_t),
...
TEST(TensorRTRTXEpTest, FP4CustomOpModel)
{
    ...
    const std::string model_name = "nv_execution_provider_fp4_dynamic_quantize_test.onnx";
    clearFileIfExists(model_name);

    model_builder::CreateFP4CustomOpModel(model_name, "nv_execution_provider_fp4_dynamic_quantize_graph");
    ASSERT_TRUE(std::filesystem::exists(model_name));
