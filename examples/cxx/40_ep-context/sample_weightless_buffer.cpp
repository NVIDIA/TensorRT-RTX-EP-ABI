...
//
// Weightless-refit EPContext example (TensorRT-RTX EP) -- BUFFER-INPUT variant.
//
// Same as sample_weightless.cpp, but the model is compiled AND reloaded FROM A MEMORY BUFFER
// (SetInputModelFromBuffer / Session-from-bytes) instead of from a file path. This unlocks the case where
// the weights file lives in a DIFFERENT folder than the .onnx: ORT's
// session.model_external_initializers_file_folder_path config (honored only on the from-buffer path) is
// set to the weights folder, so each initializer's relative external 'location' resolves there. The
// initializers stay external file refs, so the dedup callback still gets a non-null ext_info -> zero copy.
// (Loading model bytes from RAM is also the shape of the customer's decrypt-in-memory delivery.)
//
// What it does:
//   1) Compiles a WEIGHT-STRIPPED EPContext model (nv_weight_stripped_engine_enable_experimental=1). The kept weight
//      initializers are TRUE-DEDUP referenced back to the customer's EXISTING weights file at the original
//      offsets (via SetOutputModelGetInitializerLocationFunc) -> zero copy. The weights folder is supplied
//      explicitly, so the .onnx and the weights file need NOT be co-located. Non-external initializers
//      fall back to a fresh sidecar (>=threshold) or inline.
//   2) Reports sizes: original model + weights vs the EPContext model produced (shrink ratio).
//   3) Reloads the saved model FROM A BUFFER in a FRESH Env/SessionOptions that never registers the
//      original weights (weights resolved from the folder config) -- proving refit does not depend on the
//      original ONNX model at all.
//
    ...
    if (data != nullptr && nbytes >= st->threshold)
    {
        ...
        if (!st->out->good())
        {
            return api.CreateStatus(ORT_FAIL,
                                    "[sample_weightless_buffer] failed to write initializer bytes to the sidecar file");
        }
...
int main(int argc, char* argv[])
{
    ...
    // The weights file may live in a DIFFERENT folder than the .onnx. ORT resolves a model's external
    // initializer 'location' relative to the model's own dir by default, so point it at the weights folder
    // via session.model_external_initializers_file_folder_path (set on BOTH compile and reload below). This
    // lets the shipped set be a tiny .onnx + a weights file kept anywhere, not forcibly co-located.
    fs::path weights_dir = weights_model_path.has_parent_path() ? weights_model_path.parent_path() : fs::path(".");
    std::string weights_dir_str = weights_dir.string();
    ...
    try
    {
        ...
        // Load the model bytes once. The compile uses SetInputModelFromBuffer (NOT SetInputModelPath) so
        // that session.model_external_initializers_file_folder_path takes effect -- that config is only
        // honored on the from-buffer path, and it lets the weights live in a DIFFERENT folder than the
        // .onnx. The initializers stay external file refs resolved from that folder, so the dedup callback
        // still receives a non-null ext_info (reuse -> zero copy) just as with the on-disk path.
        model_buffer = LoadFileToBuffer(input_model_path);
        ...
        if (verify)
        {
            ...
            // initializer path did not actually bind the fp16 weights, so the reference was vacuous. This is
            // the reference baseline ONLY; the TEST/reload path below still exercises the from-buffer +
            // external-initializers-folder config that is this variant's whole point.
        ...
        // Resolve the model's external weights from the (possibly different) weights folder, not the .onnx dir.
        compile_session_options.AddConfigEntry(kOrtSessionOptionsModelExternalInitializersFileFolderPath,
                                               weights_dir_str.c_str());
        ...
        // copy). Because the weights folder is provided explicitly (folder-path config, set on compile and
        // reload), the output .onnx may live anywhere -- honor the user's output path; only a bare filename
        // defaults next to the input model.
        if (!output_model_path.has_parent_path())
            output_model_path =
                (input_model_path.has_parent_path() ? input_model_path.parent_path() : fs::path("."))
        ...
        // From-buffer input so the external-data folder-path config applies (see note above); the model's
        // initializers remain external refs resolved from weights_dir -> ext_info populated -> dedup.
        compile_options.SetInputModelFromBuffer(reinterpret_cast<const void*>(model_buffer.data()),
                                                model_buffer.size());
        ...
        // Resolve the reused weight refs from the weights folder (the .onnx may not be co-located with them).
        disk_session_options.AddConfigEntry(kOrtSessionOptionsModelExternalInitializersFileFolderPath,
                                            weights_dir_str.c_str());

        // Reload from a BUFFER too, so the external-data folder-path config applies at load and resolves the
        // reused weight refs from weights_dir (the .onnx need not be co-located with the weights file).
        std::vector<uint8_t> ctx_buffer = LoadFileToBuffer(output_model_path);
        ...
        double disk_time = MeasureTime([&]()
            { disk_session = Ort::Session(disk_env, ctx_buffer.data(), ctx_buffer.size(), disk_session_options); });
        std::cout << "> WEIGHTLESS RELOAD (buffer) SUCCEEDED. Load time: " << disk_time << " sec" << std::endl;
