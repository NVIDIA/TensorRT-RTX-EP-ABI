...
int main(int argc, char* argv[])
{
    ...
    {
        std::cerr << "Usage: " << argv[0] << " [input_model.onnx] [compiled_model_output.onnx] [embed_mode]"
                  << std::endl;
    ...
    try
    {
        ...
        // Normal ONNX load
        {
            ...
            session_options.AppendExecutionProvider_V2(env, {trt_device}, ep_options);
        ...
        // AOT compilation
        {
            Ort::SessionOptions session_options;
            Ort::KeyValuePairs ep_options;
            const auto runtime_cache_path = runtime_cache_dir.string();
            ep_options.Add("nv_runtime_cache_path", runtime_cache_path.c_str());
            session_options.AppendExecutionProvider_V2(env, {trt_device}, ep_options);
