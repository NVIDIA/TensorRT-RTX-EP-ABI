...
void saveOutputImage(float* pData, const char* imageFileName)
{
    ...
    if (encode_error != 0)
    {
        LOG("Failed to save output image \"{}\": {}", imageFileName, encode_error);
    }
}
...
void register_execution_providers(Ort::Env& env)
{
    ...
    {
        LOG("Failed to register {}! Skipping execution provider with error {}", providers_library.string(), e.what());
    }
}
...
Ort::ConstEpDevice find_trt_rtx_device(Ort::Env& env)
{
    ...
    {
        const bool selected = std::strcmp(device.EpName(), kNvTensorRTRTXExecutionProvider) == 0;
        LOG("{}EP device: vendor={}, ep={}, device_id=0x{:x}", selected ? "[selected] " : "", device.EpVendor(),
