...
struct ArenaConfig
{
    ...
    bool IsValid()
    {
        return initial_chunk_size_bytes > 0 && max_dead_bytes_per_chunk > 0 && initial_growth_chunk_size_bytes > 0 &&
...
public:
    ...
    ArenaImpl(AllocatorUniquePtr<OrtAllocator> allocator, const ArenaConfig& config, const OrtApi& api,
...
private:
    ...
    public:
        AllocationRegion(void* ptr, size_t memory_size, int64_t id)
            ...
            , id_(id)
        {
            ENFORCE_EP(0 == memory_size % kMinAllocationSize, __FUNCTION__);
    ...
    private:
        ...
        int IndexFor(const void* p) const
        {
            ...
            ENFORCE_EP(p_int >= base_int, "AllocationRegion::IndexFor");
            ENFORCE_EP(p_int < base_int + memory_size_, "AllocationRegion::IndexFor");
    ...
    private:
        ...
        const AllocationRegion* RegionFor(const void* p) const
        {
            ...
            ENFORCE_EP(entry != regions_.end(), "RegionManager::RegionFor Could not find Region for: " << p);
            return nullptr;
        }
    ...
    void RemoveFreeChunkIterFromBin(Bin::FreeChunkSet* free_chunks, const Bin::FreeChunkSet::iterator& c);
    ...
    ArenaImpl::Chunk* SplitFreeChunkFromBin(ArenaImpl::Bin::FreeChunkSet* free_chunks,
                                            const ArenaImpl::Bin::FreeChunkSet::iterator& citer, size_t rounded_bytes,
                                            size_t num_bytes);
    ...
    AllocatorUniquePtr<OrtAllocator> device_allocator_;
...
{
    static OrtStatus* CreateOrtArenaAllocator(AllocatorUniquePtr<OrtAllocator> allocator,
                                              const OrtKeyValuePairs* options, const OrtApi& api,
                                              const OrtLogger& logger, std::unique_ptr<ArenaAllocator>& arena_allocator)
    {
        try
        {
