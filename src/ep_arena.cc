...
ArenaImpl::ArenaImpl(AllocatorUniquePtr<OrtAllocator> allocator, const ArenaConfig& config, const OrtApi& api,
    ...
    , logger_{logger}
{
    LOG(INFO, "Creating ArenaImpl for " << allocator_name_ << " with following configs: initial_chunk_size_bytes: "
                                        << config_.initial_chunk_size_bytes << " max_dead_bytes_per_chunk: "
                                        << config_.max_dead_bytes_per_chunk << " initial_growth_chunk_size_bytes: "
                                        << config_.initial_growth_chunk_size_bytes << " max_power_of_two_extend_bytes: "
                                        << config_.max_power_of_two_extend_bytes << " memory limit: " << config_.max_mem
                                        << " arena_extend_strategy: " << config_.arena_extend_strategy);

    curr_region_allocation_bytes_ =
        RoundedBytes(std::min(config_.max_mem, static_cast<size_t>(config_.initial_chunk_size_bytes)));
    ...
    LOG(VERBOSE, "Creating " << kNumBins << " bins of max chunk size " << BinNumToSize(0) << " to "
    ...
    for (BinNum b = 0; b < kNumBins; b++)
    {
        ...
        ENFORCE_EP((BinForSize(bin_size) == BinFromIndex(b) && BinForSize(bin_size + 255) == BinFromIndex(b) &&
                    BinForSize(bin_size * 2 - 1) == BinFromIndex(b)),
                   "Invalid bin size for bin " << b);

        if (b + 1 < kNumBins)
        {
            ENFORCE_EP(BinForSize(bin_size * 2) != BinFromIndex(b), "Invalid bin size for " << b);
        }
    }
}
...
{
    ENFORCE_EP(h < chunks_.size(), "ChunkFromHandle");
    return &(chunks_[h]);
}
...
OrtStatus* ArenaImpl::Extend(size_t rounded_bytes)
{
    ...
    {
        // kBackpedalFactor is float, bytes is size_t. The result of bytes * kBackpedalFactor is float. When we cast it
        // to size_t, which is a smaller type, it could loss data. This is what C4244 complains. The
        // "static_cast<size_t>" here is to suppress the warning. C26451 suggest we may change kBackpedalFactor to
        // double to get better accuary. But if we do that, AMD GPU CI build pipeline will have an "out-of-memory"
        // error. So I choose to keep this piece of code untouched and disable the warning first.
...
#pragma warning(pop)
    ...
    // TODO: Try to merge this new region with an existing region,
...
void* ArenaImpl::Reserve(size_t size)
{
    ...
    ENFORCE_EP(reserved_chunks_.find(ptr) == reserved_chunks_.end(), __FUNCTION__);
...
size_t ArenaImpl::RequestedSize(const void* ptr)
{
    ...
    ENFORCE_EP(h != kInvalidChunkHandle, __FUNCTION__);
    ArenaImpl::Chunk* c = ChunkFromHandle(h);
...
void* ArenaImpl::AllocateRawInternal(size_t num_bytes, OrtSyncStream* stream, bool dump_log_on_failure)
{
    ...
    LOG(INFO, "Extending arena for " << allocator_name_ << ". bin_num:" << bin_num << " (requested) num_bytes: "
                                     << num_bytes << " (actual) rounded_bytes:" << rounded_bytes);
...
ArenaImpl::Chunk* ArenaImpl::FindChunkPtr(BinNum bin_num, size_t rounded_bytes, size_t num_bytes, OrtSyncStream* stream)
{
    ...
    for (; bin_num < kNumBins; bin_num++)
    {
        ...
        for (auto citer = b->free_chunks.begin(); citer != b->free_chunks.end(); ++citer)
        {
            const ArenaImpl::ChunkHandle h = (*citer);
            ArenaImpl::Chunk* chunk = ChunkFromHandle(h);
            ENFORCE_EP(!chunk->in_use(), __FUNCTION__);
...
void ArenaImpl::SplitChunk(ArenaImpl::ChunkHandle h, size_t num_bytes)
{
    ...
    Chunk* c = ChunkFromHandle(h);
    ENFORCE_EP(!c->in_use() && (c->bin_num == kInvalidBinNum), __FUNCTION__);
...
void ArenaImpl::Merge(ArenaImpl::ChunkHandle h1, ArenaImpl::ChunkHandle h2)
{
    Chunk* c1 = ChunkFromHandle(h1);
    Chunk* c2 = ChunkFromHandle(h2);
    // We can only merge chunks that are not in use.
    ENFORCE_EP(!c1->in_use() && !c2->in_use() && c1->stream == c2->stream, __FUNCTION__);
    ...
    c1->next = h3;
    ENFORCE_EP(c2->prev == h1, __FUNCTION__);
    ...
    ENFORCE_EP(c1->stream == c2->stream, "Attempting to merge chunks with different streams");
...
void ArenaImpl::RemoveFreeChunkFromBin(ArenaImpl::ChunkHandle h)
{
    Chunk* c = ChunkFromHandle(h);
    ENFORCE_EP(!c->in_use() && (c->bin_num != kInvalidBinNum), __FUNCTION__);
    ENFORCE_EP(BinFromIndex(c->bin_num)->free_chunks.erase(h) > 0, "Could not find chunk in bin");
    c->bin_num = kInvalidBinNum;
}
...
{
    Chunk* c = ChunkFromHandle(h);
    ENFORCE_EP(!c->in_use(), __FUNCTION__);
    ...
    for (const auto& region : region_manager_.regions())
    {
        ...
        while (h != kInvalidChunkHandle)
        {
            ...
            else
            {
                ...
                ENFORCE_EP(bin->free_chunks.count(h) == 1 && c->bin_num == bin_num, __FUNCTION__);
            }
...
void ArenaImpl::DumpMemoryLog(size_t num_bytes)
{
    ...
    for (BinNum bin_num = 0; bin_num < kNumBins; bin_num++)
    {
        ...
        ENFORCE_EP(b->free_chunks.size() == bin_info.total_chunks_in_bin - bin_info.total_chunks_in_use, __FUNCTION__);

        if (bin_info.total_chunks_in_bin > 0)
        {
            LOG(INFO, b->bin_size << ": Chunks " << bin_info.total_chunks_in_use << "/" << bin_info.total_chunks_in_bin
                                  << ". Bytes " << bin_info.total_bytes_in_use << "/" << bin_info.total_bytes_in_bin
                                  << ". "
    ...
    LOG(INFO, "Bin for " << num_bytes << " bytes has max bytes of " << b->bin_size << ", Chunk State: ");
    ...
    for (const auto& region : region_manager_.regions())
    {
        ...
        while (h != kInvalidChunkHandle)
        {
            ...
            LOG(INFO, (c->in_use() ? "  Chunk" : "  Free ") << " at " << c->ptr << " of size " << c->size);
            h = c->next;
        }
    }
    ...
    {
        LOG(INFO, "  " << it.second << " chunks of size " << it.first << ". Total " << it.first * it.second);
