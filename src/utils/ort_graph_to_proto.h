...
// Helper function to calculate the indegree of a node
// Indegree is the count of incoming edges from other nodes
static Ort::Status CalculateNodeIndegree(const Ort::ConstNode& node, size_t& indegree) {
  indegree = 0;
  ...
  for (auto input : inputs) {
    ...
    ORT_EP_UTILS_CXX_RETURN_IF_ERROR(Ort::GetApi().ValueInfo_GetValueProducer(input, &producer, &producer_index));
    
    // Only count inputs with non-null producers (edges from other nodes)
    if (producer != nullptr) {
      indegree++;
    }
  }
...
static Ort::Status KahnsTopologicalSort(
    const std::function<void(Ort::ConstNode node)>& enter,
    const std::vector<Ort::ConstNode>& nodes) {

  std::map<size_t, size_t> in_degree;
  std::vector<size_t> topo_order;

  // Node comparison function: prioritize Shape/Size, then node ID
  auto node_comp = [](const Ort::ConstNode& n1, const Ort::ConstNode& n2) -> bool {
    auto is_high_pri = [](const Ort::ConstNode& n) -> bool {
    ...
    bool n1_high = is_high_pri(n1);
    bool n2_high = is_high_pri(n2);
    if (n1_high != n2_high) return n2_high;
    return n1.GetId() > n2.GetId();
  };

  // Compare node indices via node_comp
  VisitorPriorityQueue<Ort::ConstNode> to_visit(node_comp);
  ...
  for (auto& node : nodes) {
    size_t indegree = 0;
    ORT_EP_UTILS_CXX_RETURN_IF_ERROR(CalculateNodeIndegree(node, indegree));
    in_degree[node.GetId()] = indegree;
    if (indegree == 0) {
      to_visit.push(node);
    }
  }
  ...
  while (!to_visit.empty()) {
    const Ort::ConstNode current = to_visit.top();
    to_visit.pop();
    ...
    for (auto output : current.GetOutputs()) {
      ...
      for (auto consumer_info : consumer_infos) {
        ...
        if (consumer == nullptr) {
          continue;  // Skip if no producer (e.g., graph input)
        }
        auto& prod_in_deg = in_degree[consumer.GetId()];
        prod_in_deg--;
        if (prod_in_deg == 0) {
    ...
    topo_order.push_back(current.GetId());
  }

  if (number_of_nodes != topo_order.size()) {
    ORT_EP_UTILS_C_RETURN_IF(true, "Some nodes are not included in the topological sort, graph have a cycle.");
  }
  return Ort::Status{nullptr};
}
...
Ort::Status OrtGraphToProto(const OrtGraph& graph,
  ...
  try {
    ...
    std::vector<Ort::ConstNode> nodes = ort_graph.GetNodes();
     ...
     std::vector<Ort::ConstNode> nodes_in_topological_order_with_priority_;
     nodes_in_topological_order_with_priority_.reserve(nodes.size());
     
     ORT_EP_UTILS_CXX_RETURN_IF_ERROR(KahnsTopologicalSort(
       [&nodes_in_topological_order_with_priority_](Ort::ConstNode node) {
         nodes_in_topological_order_with_priority_.push_back(node);
       },
       nodes));
    ...
    for (const auto& ort_node : nodes_in_topological_order_with_priority_) {
      ...
      node_proto->set_doc_string(std::to_string(ort_node.GetId()));
