...
public:
    ...
    static std::filesystem::path JoinPath(const std::filesystem::path& path1, const std::filesystem::path& path2)
    {
        return path1 / path2;
    }
    ...
    static std::wstring GetCurrentModuleDirectory(const void* moduleFunc = nullptr)
    {
        ...
        if (!GetModuleHandleExW(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS | GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT,
                                reinterpret_cast<LPCWSTR>(addrToUse), &hModule))
        {
            return L"";
        }
...
//!
//! \param path File to read. A missing file is treated as a cache miss.
//! \param ort_api ORT API used to report file and allocation failures.
//! \return The file contents, or an empty vector when the file does not exist or is empty.
//!
inline std::vector<char> ReadFile(const std::filesystem::path& path, const OrtApi& ort_api)
{
    if (!std::filesystem::exists(path))
    {
        // Cache miss: return empty so the caller proceeds.
        return {};
    }
    ...
    {
        std::string message = "Failed to allocate memory for file read (" + std::to_string(usize) +
                              " bytes): " + PathToUTF8String(path.native());
        Ort::ThrowOnError(ort_api.CreateStatus(ORT_EP_FAIL, message.c_str()));
    }
...
inline std::vector<char> ReadFile(const std::string&, const OrtApi&) = delete;

//!
//! \brief Writes a binary buffer to a file, replacing any existing contents.
//!
...
//! \param ort_api ORT API used to report file-open or file-creation failures.
//!
inline void WriteFile(const std::filesystem::path& path, const void* data, size_t size, const OrtApi& ort_api)
{
    if (std::filesystem::exists(path))
    {
...
//! \brief Prevents narrow-string paths from bypassing explicit filesystem path handling.
inline void WriteFile(const std::string&, const void*, size_t, const OrtApi&) = delete;
